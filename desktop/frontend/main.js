...
console.log('[GT] main.js 已加载');
window.__GT_VER = 'restored-2';

// WebView2 中 window.alert 无对话框处理器会永久阻塞 JS 线程（白屏），必须拦掉
window.alert = () => console.warn('[GT] alert 被拦截（WebView2 不支持）');
window.addEventListener('error', (e) => console.error('[GT] JS 错误:', e.message, e.filename, e.lineno));
window.addEventListener('unhandledrejection', (e) => console.error('[GT] Promise 异常:', e.reason));
...
// Wails v3 runtime.js 是 ES Module（顶层 export），必须 type="module" 加载；
// 且模块求值后 window.wails.Call 仍可能延迟挂载，注入后轮询等待直到可用
function ensureWailsRuntime(onReady, attempt = 0) {
    if (window.wails && window.wails.Call) return onReady();
    if (attempt === 0) {
        const s = document.createElement('script');
        s.type = 'module';
        s.src = '/wails/runtime.js';
        s.onerror = () => console.warn('[GT] Wails runtime 加载失败');
        document.head.appendChild(s);
    }
    if (attempt > 100) { console.warn('[GT] wails.Call 未就绪，放弃'); return; }
    setTimeout(() => ensureWailsRuntime(onReady, attempt + 1), 100);
}
...
// 绑定所有的 DOM 动作事件监听器（WebView2 下 DOMContentLoaded 可能早于脚本，需守卫）
function bootApp() {
    // 核心开启/关闭
